...
Group = (function() {
  function Group(nodes) {
    this.selection = nodes.append('g').attr('transform', this.translate).attr('data-toggle', 'popover').attr('title', this.title).attr('data-content', this.dataContent).attr('data-placement', this.placement).on('mouseover', this.mouseover).on('mouseout', this.mouseout).on('click', this.click);
    this.callbacks = {};
    this.force = null;
    this;
  }
  ...
  Group.prototype.translate = function(data, index) {
  ...
  Group.prototype.all = function(width, height) {
    ...
    this.force = d3.layout.force().nodes(this.selection.data()).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).on('tick', tick).on('end', end).start();
    this.selection.call(this.force.drag);
    return this;
  };

  Group.prototype.boundary = function() {
    var data, x, y;
    data = this.selection.data().slice(1);
  ...
  Group.prototype.total = function() {
    return this.selection.data().length;
  };
  ...
  Group.array = ['SPEC', 'CW', 'T', 'L', 'F2', 'F1', 'Z', 'C', 'D'];

  Group.columns = function(width) {
    var step;
    step = width / 10.0;
    return d3.scale.ordinal().domain(Group.array).range(d3.range(step, step * 10, step));
  };

  Group.prototype.byCategory = function(width, height) {
    var columns, group, i, len, posy, ref, text;
    posy = {};
