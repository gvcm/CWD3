...
Group = (function() {
  ...
  Group.prototype.cluster = function(width, height) {
    ...
    this.data[0].fixed = true;
    this.force = d3.layout.force().nodes(this.data).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).on('tick', tick).on('end', end).start();
    this.nodes.call(this.force.drag);
    return this;
  };

  Group.prototype.boundary = function() {
    var boundary, i, len, ref, row;
    boundary = {
      x1: this.data[0].x,
      y1: this.data[0].y,
      x2: this.data[0].x,
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      row = ref[i];
      if (row.x < boundary.x1) {
        boundary.x1 = row.x;
      }
      if (row.y < boundary.y1) {
        boundary.y1 = row.y;
      }
      if (row.x > boundary.x2) {
        boundary.x2 = row.x;
      }
      if (row.y > boundary.y2) {
        boundary.y2 = row.y;
      }
    }
    return boundary;
  ...
  View.prototype.render = function(data) {
    ...
    return group.cluster(this.width, this.height).on('forceEnd', (function(_this) {
      return function() {
        ...
        return total.text("TOTAL=" + (group.total()));
      };
    })(this));
  };
