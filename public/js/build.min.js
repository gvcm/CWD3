...
View = (function() {
  ...
  View.prototype.criteriaTab = function() {
    var _, bubble, clonedRow, counter, criteriaGroup, d, f, i, k, len, nodeGroup, ref, ref1, results, row, v;
    criteriaGroup = {};
    ref = Criteria.hashMap;
    for (k in ref) {
      _ = ref[k];
    ...
    for (k in criteriaGroup) {
      ...
      if (d.length > 0) {
        ...
        nodeGroup = this.selection.append('g').attr('class', 'bubble-' + counter).attr('transform', (function(_this) {
          return function() {
            var rx, ry;
            rx = Math.random() * _this.width;
            ry = Math.random() * _this.height;
            return "translate(" + rx + "," + ry + ")";
          };
        })(this));
        bubble = new Bubble(nodeGroup.selectAll('g.bubble').data(d).enter());
        results.push(bubble.byDefault(Math.random() * this.width, Math.random() * this.height));
      } else {
        results.push(void 0);
      }
    }
