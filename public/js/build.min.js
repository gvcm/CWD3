...
Group = (function() {
  ...
  Group.prototype.all = function(width, height) {
    ...
    spec.attr('transform', function() {
      return "translate(" + ((width / 12.0) * 10) + "," + (height / 2.0) + ")";
    });
    nodes = this.selection.filter(function(data) {
      return data.group !== 'SPEC';
    });
    this.force = d3.layout.force().nodes(nodes.data()).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).on('tick', tick).on('end', end).start();
...
View = (function() {
  function View(selector, data) {
    ...
    this.element = d3.select(selector).append('svg').attr('width', this.width).attr('height', this.height);
    this.group = new Group(this.element.selectAll('.node').data(data).enter());
  ...
  View.prototype.categoryTab = function() {
    ...
    group = new Group(this.element.selectAll('.node').data(this.data).enter());
