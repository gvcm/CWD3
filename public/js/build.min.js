...
Bubble = (function() {
  function Bubble(nodes, bubbleClass) {
    this.opacity = bind(this.opacity, this);
    if (bubbleClass == null) {
      bubbleClass = 'bubble';
    }
    this.selection = nodes.append('g').attr('class', bubbleClass).attr('data-toggle', 'popover').attr('title', this.title).attr('data-content', this.dataContent).attr('data-placement', this.placement).attr('opacity', this.opacity).on('mouseover', this.mouseover).on('mouseout', this.mouseout).on('click', this.click);
    this.callbacks = {};
    this.force = null;
    this.circle = new Circle(this.selection);
    this.label = new Label(this.selection);
    this.selection.attr('transform', function(data) {
      var rx, ry, theta;
  ...
  Bubble.prototype.opacity = function(data) {
    if (View.showRef()) {
      return 1;
    }
    if (data.group === 'SPEC' || data.group === 'CW') {
      return 0;
    } else {
      return 1;
    }
  ...
  Bubble.prototype.mouseover = function(x) {
    var bubble, bubbleNode, circle;
    bubble = d3.select(this);
    bubbleNode = bubble.node();
    bubbleNode.parentNode.appendChild(bubbleNode);
    circle = bubble.selectAll('circle');
    return circle.attr('data-prev-stroke', circle.attr('stroke')).attr('data-prev-stroke-width', circle.attr('stroke-width')).attr('stroke', 'black').attr('stroke-width', 3);
  };

  Bubble.prototype.mouseout = function(x) {
    var bubble, bubbleNode, circle, parentNode;
    bubble = d3.select(this);
    bubbleNode = bubble.node();
    parentNode = bubbleNode.parentNode;
    parentNode.insertBefore(bubbleNode, parentNode.firstChild);
    circle = bubble.selectAll('circle');
    return circle.attr('stroke', circle.attr('data-prev-stroke')).attr('stroke-width', circle.attr('data-prev-stroke-width'));
  };

  Bubble.prototype.placement = function(data, index) {
    return 'bottom';
  };

  Bubble.prototype.title = function(data, index) {
    if (data.group !== 'SPEC') {
      return data.title;
    }
  };

  Bubble.prototype.click = function(data) {
  ...
  Bubble.prototype.dataContent = function(data, index) {
    if ((data.total != null) && (data.score != null) && (data.group != null) && (data.description != null)) {
      return "<table> <tr><td style=\"text-align:right\"><strong>Total:</strong></td><td>&nbsp;&nbsp;&nbsp;" + data.total + "</td></tr> <tr><td style=\"text-align:right\"><strong>Scores 1–5:</strong></td><td>&nbsp;&nbsp;&nbsp;" + (data.score.join(', ')) + "</td></tr> <tr><td style=\"text-align:right\"><strong>Bubble:</strong></td><td>&nbsp;&nbsp;&nbsp;" + data.group + "</td></tr> <tr><td colspan=\"2\">" + data.description + "</td></tr> </table>";
    } else if (data.title != null) {
      return data.title;
    }
  };

  Bubble.prototype.transform = function(func) {
    return this.selection.attr('transform', func);
  };

  Bubble.prototype.charge = function(data, index) {
    return -(data.total + 1) * 25;
  };

  Bubble.prototype.byDefault = function(width, height) {
    var end, nodes, showRef, spec, tick;
    showRef = View.showRef();
    if (showRef) {
      spec = this.selection.filter(function(data) {
        return data.group === 'SPEC';
      });
      spec.transition().duration(500).attr('transform', function() {
        return "translate(" + ((width / 12.0) * 10) + "," + (height / 2.0) + ")";
      });
    }
    nodes = this.selection.filter(function(data) {
      if (showRef) {
        return data.group !== 'SPEC';
      } else {
        return data.group !== 'SPEC' && data.group !== 'CW';
      }
    });
    tick = (function(_this) {
      return function(e) {
        if (e.alpha < 0.04) {
          _this.force.stop();
          nodes.transition().duration(2000).attr('transform', function(data) {
      ...
      return function(e) {
        if (_this.callbacks['forceEnd'] != null) {
          return _this.callbacks['forceEnd']();
        }
      };
    })(this);
    this.force = d3.layout.force().nodes(nodes.data()).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).friction(0.8).on('tick', tick).on('end', end).start();
    return this;
  };

  Bubble.prototype.boundary = function() {
    var data, x, y;
    data = this.selection.filter(function(data) {
      return data.group !== 'SPEC';
    }).data();
  ...
  Bubble.prototype.total = function() {
    return this.selection.data().length - 1;
  };

  Bubble.prototype.on = function(event, callback) {
    this.callbacks[event] = callback;
    return this;
  };

  Bubble.categories = ['G [1] - Gold Standard', 'CW [1] - CrystalWalk', 'T [26] - Tested Software', 'L [2] - Discard: License not provided', 'F2 [16] - Discard: OOS after Test', 'F1 [133] - Discard: OOS after Match', 'Z [604] - Discard: Out Of Scope', 'C [19] - Discard: Discontinued', 'D [4] - Discard: Duplicated'];

  Bubble.columns = function(width) {
    var step;
    step = width / 10.0;
    return d3.scale.ordinal().domain(Bubble.categories).range(d3.range(step, step * 10, step));
  };

  Bubble.prototype.byCategory = function(width, height) {
    var category, columns, i, len, posy, ref, svg, text;
    posy = {};
    columns = Bubble.columns(width);
    svg = d3.select('svg');
    ref = Bubble.categories;
    ...
    return this.selection.transition().duration(3000).attr('transform', (function(_this) {
      return function(data, index) {
        posy[data.group] = posy[data.group] != null ? posy[data.group] + Math.sqrt(data.value * 500) : 1;
        return "translate(" + (columns(data.group)) + "," + (posy[data.group] + 300) + ")";
      };
    })(this));
  };

  Bubble.prototype.byPopularity = function(width, height) {
    var category, columns, i, len, posy, ref, text;
    posy = {};
    columns = Bubble.columns(width);
    ref = Bubble.categories;
    for (i = 0, len = ref.length; i < len; i++) {
  ...
  Bubble.prototype.byPoints = function(width, height) {};
...
Circle = (function() {
  function Circle(nodes) {
    this.radius = bind(this.radius, this);
    this.selection = nodes.append('circle').filter(function(data) {
      return !data.children;
    }).attr('r', 0).attr('fill', this.fill).attr('stroke-width', 1).attr('stroke', 'black');
  }

  Circle.prototype.radius = function(data) {
    if (data.r != null) {
      return Circle.linearRadiusScale(data.r);
    } else {
      return Circle.sqrtRadiusScale(data.scoreN);
    }
  };

  Circle.prototype.fill = function(data) {
    return Circle.backgroundPallete(data.group);
  };

  Circle.prototype.show = function() {
    return this.selection.transition().duration(3000).attr('r', this.radius);
  };

  Circle.prototype.hide = function() {
    return this.selection.attr('r', 0);
  };

  Circle.backgroundPallete = d3.scale.ordinal().domain(['SPEC', 'CW', 'T', 'L', 'F2', 'F1', 'Z', 'C', 'D']).range(['#ff0000', '#000000', '#595959', '#85898f', '#e89e78', '#3f9657', '#b3c841', '#efb052', '#5d3b5a']);

  Circle.linearRadiusScale = d3.scale.linear();

  Circle.sqrtRadiusScale = d3.scale.sqrt().domain([0, 100]).range([5, 5 + (Math.sqrt(100 / Math.PI) * 10)]);
...
Criteria = (function() {
  ...
  Criteria.hashMap = {
	'1_2_3': '1.2.3 Update Frequency: Active or Dynamic',
    '1_3_1': '1.3.1 Development Platform: HTML',
    '1_3': '1.3 Application Portability: High',
    '1_5_1': '1.5.1 Use, Distribution and Modification License: Open-Source',
    '1_5_2': '1.5.2 Collaborative Development Model Support: Yes',
    '1_6_1': '1.6.1 Base Devices (HID): Mouse, Keyboard and Trackeables',
    '1_6_2': '1.6.2 Advanced Interaction Device Support: 3D Monitor and HMD',
    '2_1': '2.1 Synthesis Method',
    '2_2': '2.2 Interactive Synthesis Process: Yes',
    '2_3': '2.3 Restriction or Limitation in Synthesis Process: No',
    '2_4': '2.4 Interactive Atom Selection and Editing: Yes',
    '3_1_1': '3.1.1 Model of Molecular Atomic Representation: Rigid Spheres',
    '3_1_2': '3.1.2 Model of Unit Cell Representation: Whole, partial & interstices',
    '3_1_3': '3.1.3 Miller Planes: Yes',
    '3_1_4': '3.1.4 Miller Directions: Yes',
    '3_1_5_1': '3.1.5.1 Multiple Cells: Yes',
    '3_1_5_2': '3.1.5.2 Cuts: Yes',
    '3_1_5_3': '3.1.5.3 Axis Vectors: Yes',
    '3_1_5_4': '3.1.5.4 Other Auxiliary Resources: Lattice faces & borders',
    '3_2_1': '3.2.1 Visual Perception Features: Shapes & perspective, color, opacity, luminosity, depth & sound',
    '3_2_2': '3.2.2 Projection Type: Conic & Axonometric',
    '3_2_3': '3.2.3 Stereographic Support: Anagliph, Side-by-side & HMD',
    '3_2_4': '3.2.4 Rendering Modes: Flat, Sketch/Cartoon, Realistic',
    '4_1': '4.1 Interface: GUI & Ordered',
    '4_2': '4.2 Base Interaction: Translation, Rotation & Zoom',
    '4_3_1': '4.3.1 Automatic rotations: Yes',
    '4_3_2': '4.3.2 Guided animations: Yes',
    '4_3_3': '4.3.3 Scale Transition: Yes',
    '4_3_4': '4.3.4 Dynamic Occlusion Management: Yes',
    '4_3_5': '4.3.5 Preconfigured Viewpoints: Atoms, directions & Orthogonal perspectives',
    '5_1': '5.1 Knowledge Required from User: Basic Material Sciences',
    '5_2_1': '5.2.1 Display support / External portability: Full application, 3D Model',
    '5_2_2': '5.2.2 2D printing support: Bitmap image',
    '5_2_3': '5.2.3 3D printing support: STL model',
    '5_2_4': '5.2.4 Internet Publishing Platform: Yes',
    '5_3_1': '5.3.1 Support for didactic narrative: Yes',
    '5_3_2': '5.3.2 Library of crystallographic structures: Yes',
    '5_3_3': '5.3.3 Incremental Construction and Visualization of Structures: Yes',
    '5_4_1': '5.4.1 Available Support: Yes',
    '5_4_2': '5.4.2 Available Documentation: Yes '
  };
...
Hierarchy = (function() {
  ...
  Hierarchy.prototype.root = function(rootName) {
    this.rootName = rootName;
    return this;
  };

  Hierarchy.prototype.group = function(groupAttr) {
    this.groupAttr = groupAttr;
    return this;
  };

  Hierarchy.prototype.name = function(nameAttr) {
    this.nameAttr = nameAttr;
    return this;
  };

  Hierarchy.prototype.value = function(valueAttr) {
    this.valueAttr = valueAttr;
    return this;
  };

  Hierarchy.prototype.ungroupedNodes = function() {
    var i, len, node, ref, row;
    this.tree['name'] = this.rootName;
    this.tree['children'] = [];
    ref = this.data;
    for (i = 0, len = ref.length; i < len; i++) {
      row = ref[i];
      node = row;
      node['name'] = row[this.nameAttr];
      node['value'] = row[this.valueAttr];
      this.tree['children'].push(node);
    }
    return this.tree;
  };

  Hierarchy.prototype.groupedNodes = function() {
    var attr, childObject, group, groupKey, groups, i, k, len, ref, row, value;
    groups = {};
    ref = this.data;
    for (i = 0, len = ref.length; i < len; i++) {
      row = ref[i];
      groupKey = row[this.groupAttr];
      ...
      childObject['name'] = row[this.nameAttr];
      childObject['value'] = row[this.valueAttr];
    }
    this.tree['name'] = this.rootName;
    this.tree['children'] = (function() {
      var results;
      results = [];
      for (k in groups) {
        group = groups[k];
  ...
  Hierarchy.prototype.nodes = function() {
...
Label = (function() {
  function Label(nodes) {
    this.selection = nodes.append('text').attr('opacity', 0).attr('text-anchor', 'middle').attr('font-size', this.fontSize).attr('fill', this.color);
    this.selection.append('tspan').text(this.short);
    this.selection.append('tspan').attr('x', 0).attr('dy', this.lineHeight).text(this.scoreN);
  }

  Label.prototype.show = function() {
    return this.selection.transition().duration(1000).attr('opacity', 1);
  };

  Label.prototype.hide = function() {
    return this.selection.attr('opacity', 0);
  };

  Label.prototype.short = function(data, index) {
    if (index < Label.shortLabels.length) {
      return "[" + Label.shortLabels[index] + "]";
    }
  };

  Label.prototype.scoreN = function(data, index) {
    if ((data.scoreN != null) && data.scoreN > 0 && index < Label.shortLabels.length) {
      return data.scoreN + "%";
    }
  };

  Label.prototype.lineHeight = function(data, index) {
    if ((data.scoreN != null) && data.scoreN > 0) {
      return Label.fontSizeScale(data.scoreN) * 12;
    } else {
      return 0;
    }
  };

  Label.fontSizeScale = d3.scale.log();

  Label.prototype.fontSize = function(data, index) {
    if ((data.scoreN != null) && data.scoreN > 0) {
      return Label.fontSizeScale(data.
N) * 8;
    } else {
      return 0;
    }
  ...
  Label.prototype.color = function(data, index) {
    if (index === 1 || data.group === 'T') {
      return '#FFFFFF';
    }
  };

  Label.shortLabels = ['SPEC16', 'CWGL16', 'CMAK15', 'JMOL15', 'VEST14', 'DIAM15', 'XCRY14', 'MERC15', 'PYMO18', 'DRAW11', 'CARI04', 'BALL08', 'LATT04', 'SCHA14', 'ATOM11', 'SHEL15', 'RASM09', 'CHEM15', 'QUTE07', 'XTAL03', 'CRYS07', 'POWD00', 'PLAT13', 'GRET00', 'JAMM02', 'ORTE14', 'OSCA15', 'STRU05'];
...
Row = (function() {
  function Row(record) {
    var _, i, k, num, ref, str;
    this.scoreN = parseInt(record.scoreN, 10);
    this.value = Row.weightScale(this.scoreN) + 1;
    this.score = [];
    this.scoreXN = [];
    for (num = i = 1; i <= 5; num = ++i) {
      str = record['score' + num];
      this.score[num - 1] = str.length === 0 ? 0 : parseInt(str, 10);
      str = record['score' + num + 'N'];
      this.scoreXN[num - 1] = str.length === 0 ? 0 : parseInt(str, 10);
    }
    this.total = parseInt(record.score, 10);
    this.title = record.software;
    this.group = record.group;
    this.description = record.description;
    this.link = record.link;
    this.criteriaKeys = [];
    ref = Criteria.hashMap;
    for (k in ref) {
      _ = ref[k];
      if (record[k] === '1') {
        this.criteriaKeys.push(k);
      }
    }
    this.x = 0;
    this.y = 0;
  ...
  Row.weightScale = d3.scale.pow().exponent(0.8);
...
Score = (function() {
  function Score(nodes) {
    this.selection = nodes.append('g');
    this.circle = new Circle(this.selection);
    this.circle.show();
  }

  Score.prototype.byDefault = function(width, height) {
    var columnSize;
    columnSize = width / 10;
    return this.selection.attr('transform', function(data) {
      var rx, ry;
...
Table = (function() {
  function Table(file, callback) {
    this.file = file;
    this.rows = [];
    this.callback = callback;
    d3.csv(file, this.load.bind(this));
  }

  Table.prototype.load = function(data) {
    data.forEach(this.createRow.bind(this));
    return this.callback();
  };

  Table.prototype.createRow = function(record) {
    return this.rows.push(new Row(record));
  };
...
Text = (function() {
  function Text(parent) {
    this.selection = parent.append('text').classed('volatile', true).attr('font-weight', 'bold').text('');
    this;
  }

  Text.prototype.translate = function(x, y) {
    return this.selection.attr('transform', "translate(" + x + "," + y + ")");
  };
...
View = (function() {
  function View(selector, data) {
    ...
    this.$container.height($(window).height() - this.$container.position().top);
    this.data = data;
    this.width = this.$container.width();
    this.height = this.$container.height() * 2;
    this.selection = d3.select(selector).append('svg').attr('width', this.width).attr('height', this.height);
  ...
  View.prototype.setHeight = function(height) {
    ...
    return this.selection.attr('height', height);
  };

  View.prototype.render = function(tab) {
    if (tab == null) {
      tab = 'default';
    }
  ...
  View.prototype.update = function() {
    return this.render(this.currentTab);
  };

  View.showRef = function() {
    return $('#show-ref').prop('checked');
  };
  ...
  View.prototype.center = function() {
    return window.scrollTo(0, this.height / 4);
  };

  View.prototype.scrollLock = function(lock) {
    return $('body').css('overflow-x', 'hidden').css('overflow-y', (lock ? 'hidden' : 'scroll'));
  };

  View.prototype.defaultTab = function() {
    var bubble, total;
    total = new Text(this.selection);
    this.center();
    this.scrollLock(true);
    bubble = new Bubble(this.selection.selectAll('g').data(this.data).enter());
    return bubble.byDefault(this.width, this.height).on('forceEnd', (function(_this) {
      return function() {
        var boundary;
  ...
  View.prototype.categoryTab = function() {
    var bubble;
    this.top();
    this.scrollLock(true);
    bubble = new Bubble(this.selection.selectAll('g').data(this.data).enter());
    bubble.byCategory(this.width, this.height);
    bubble.show();
    this.setHeight(14000);
    return this.scrollLock(false);
  };

  View.prototype.getSoftwareTitles = function() {
    var i, len, ref, ref1, row, softwareTitles;
    softwareTitles = [];
    ref = this.data;
    for (i = 0, len = ref.length; i < len; i++) {
      row = ref[i];
      if ((ref1 = row.group) === 'SPEC' || ref1 === 'CW' || ref1 === 'T') {
  ...
  View.prototype.getScoreData = function() {
    var dataByScore, i, n, ref, ref1, row, y, yInteger;
    dataByScore = [];
    ref = this.data;
    for (y in ref) {
      row = ref[y];
  ...
  View.prototype.scoreTab = function() {
    var columnSize, criteriaText, i, l, len, n, positionX, positionY, ref, score, title, total;
    this.top();
    this.scrollLock(true);
    score = new Score(this.selection.selectAll('g').data(this.getScoreData()).enter());
    score.byDefault(this.width, this.height);
    total = new Text(this.selection);
    positionY = 250;
    ref = this.getSoftwareTitles();
    for (i = 0, len = ref.length; i < len; i++) {
      title = ref[i];
      criteriaText = new Text(this.selection);
      criteriaText.text(title);
      criteriaText.translate(50, positionY);
      criteriaText.show();
      positionY += 100;
    }
    columnSize = this.width / 10;
...
//    }
      ...
      criteriaText = new Text(this.selection);
      criteriaText.text('Total Criteria Sum');
      criteriaText.translate(440,140);
      criteriaText.show();

      criteriaText = new Text(this.selection);
      criteriaText.text('Technology Criteria (1) Sum');
      criteriaText.translate(670,140);
      criteriaText.show();

      criteriaText = new Text(this.selection);
      criteriaText.text('Synthesis Criteria (2) Sum');
      criteriaText.translate(940,140);
      criteriaText.show();

      criteriaText = new Text(this.selection);
      criteriaText.text('Visualization Criteria (3) Sum');
      criteriaText.translate(1180,140);
      criteriaText.show();

      criteriaText = new Text(this.selection);
      criteriaText.text('Navigation & Interactivity Criteria (4) Sum');
      criteriaText.translate(1390,140);
      criteriaText.show();

	  criteriaText = new Text(this.selection);
      criteriaText.text('Didactic Criteria (5) Sum');
      criteriaText.translate(1690,140);
      criteriaText.show();
	  
	  this.setHeight(positionY + 250);
    return this.scrollLock(false);
  };

  View.prototype.getDataGroupedByCriteria = function() {
    var _, clonedRow, dataGroup, f, i, k, len, ref, ref1, row, v;
    dataGroup = {};
    ref = Criteria.hashMap;
    for (k in ref) {
      _ = ref[k];
      ...
      for (i = 0, len = ref1.length; i < len; i++) {
        ...
        if (indexOf.call(row.criteriaKeys, k) >= 0) {
  ...
  View.prototype.buildBubbleGroups = function(data) {
    var bubble, column, columnSize, counter, criteriaText, d, k, marginSize, maxY, nodeGroup, numberOfColumns, numberOfRows, row, rowSize;
    counter = 0;
    marginSize = 50;
    numberOfColumns = Math.ceil((this.width - (2 * marginSize)) / 600);
    columnSize = this.width / numberOfColumns;
    numberOfRows = Math.ceil(this.height / 500);
    rowSize = this.height / numberOfRows;
    maxY = 0;
    for (k in data) {
      d = data[k];
      ...
      counter += 1;
      nodeGroup = this.selection.append('g').attr('class', 'bubble-' + counter).attr('transform', (function(_this) {
        return function() {
          var rx, ry;
          rx = column * columnSize + (column > 0 ? 0 : marginSize);
    ...
    return this.setHeight(maxY + rowSize);
  };

  View.prototype.criteriaTab = function() {
    return this.buildBubbleGroups(this.getDataGroupedByCriteria());
  };

  View.prototype.popularityTab = function() {
    var d, data, i, j, k, len, sortable, sortedData;
    data = this.getDataGroupedByCriteria();
    sortable = [];
    for (k in data) {
      d = data[k];
    ...
    return this.buildBubbleGroups(sortedData);
  };
...
table = new Table('data.csv', function() {
  var view;
  view = new View('#view', table.rows);
  $('[data-view]').click(function() {
    return view.render($(this).data('view'));
  });
  return $('#show-ref').click(function() {
    return view.update();
  });
});
