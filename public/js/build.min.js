...
Circle = (function() {
  ...
  Circle.prototype.radius = function(data) {
    return Circle.radiusScale(data.scoreN);
  };
...
Group = (function() {
  function Group(nodes) {
    this.element = nodes.append('g').attr('data-toggle', 'popover').attr('title', this.title).attr('data-content', this.dataContent).attr('data-placement', this.placement).on('mouseover', this.mouseover).on('mouseout', this.mouseout).on('click', this.click);
    this.callbacks = {};
    this.force = null;
    this;
  }
...
View = (function() {
  ...
  View.prototype.tabs = function() {
    return {
      all: (function(_this) {
        return function() {
          var circle, group, i, label, len, ref, row, total;
          ref = _this.data;
          for (i = 0, len = ref.length; i < len; i++) {
            row = ref[i];
            row.x = Math.random() * _this.width;
            row.y = Math.random() * _this.height;
          }
          group = new Group(_this.element.selectAll('.node').data(_this.data).enter());
          circle = group.append(new Circle());
          label = group.append(new Label());
          total = new Text(_this.element);
          _this.center();
          _this.scrollLock(true);
          circle.show();
          label.show();
          return group.cluster(_this.width, _this.height).on('forceEnd', function() {
            var b;
            b = group.boundary();
