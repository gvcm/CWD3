...
Group = (function() {
  ...
  Group.prototype.cluster = function(width, height) {
    ...
    force = d3.layout.force().nodes(this.data).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).on('tick', tick).start();
    return this.nodes.call(force.drag);
  };
...
Text = (function() {
  ...
  Text.prototype.build = function(parent) {
    ...
    this.element.append('tspan').text(this.textLabel);
    return this.element.append('tspan').attr('x', 0).attr('dy', this.textLineHeight).text(this.textWeight);
  };

  Text.prototype.show = function() {
    return this.element.transition().duration(3000).attr('opacity', 1);
  };

  Text.prototype.textLabel = function(data, index) {
    if (index < Text.shortLabels.length) {
      return "[" + Text.shortLabels[index] + "]";
    }
  };

  Text.prototype.textWeight = function(data, index) {
    if ((data.weight != null) && data.weight > 0 && index < Text.shortLabels.length) {
      return data.weight + "%";
    }
  };

  Text.prototype.textLineHeight = function(data, index) {
    if ((data.weight != null) && data.weight > 0) {
      return Text.fontSizeScale(data.weight) * 12;
    } else {
      return 0;
    }
  };

  Text.fontSizeScale = d3.scale.log();

  Text.prototype.fontSize = function(data, index) {
    if ((data.weight != null) && data.weight > 0) {
      return Text.fontSizeScale(data.weight) * 8;
    } else {
      return 0;
    }
  };

  Text.prototype.color = function(data, index) {
    if (index === 1 || data.group === 'T') {
      return '#FFFFFF';
    }
  };

  Text.shortLabels = ['SPEC16', 'CWGL16', 'CMAK15', 'JMOL15', 'VEST14', 'DIAM15', 'XCRY14', 'MERC15', 'PYMO18', 'DRAW11', 'CARI04', 'BALL08', 'LATT04', 'SCHA14', 'ATOM11', 'SHEL15', 'RASM09', 'CHEM15', 'QUTE07', 'XTAL03', 'CRYS07', 'POWD00', 'PLAT13', 'GRET00', 'JAMM02', 'ORTE14', 'OSCA15', 'STRU05'];
...
View = (function() {
  ...
  View.prototype.render = function(data) {
    ...
    circle.show();
    text.show();
    return group.cluster(this.width, this.height);
  };
