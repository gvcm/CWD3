...
Group = (function() {
  function Group(nodes) {
    this.element = nodes.append('g').attr('transform', this.translate).attr('data-toggle', 'popover').attr('title', this.title).attr('data-content', this.dataContent).attr('data-placement', this.placement).on('mouseover', this.mouseover).on('mouseout', this.mouseout).on('click', this.click);
    this.callbacks = {};
    this.force = null;
    this;
  }
  ...
  Group.prototype.mouseout = function(x) {
    var circle, group, groupElement, parentNode;
    group = d3.select(this);
    groupElement = group[0][0];
  ...
  Group.prototype.transform = function(func) {
    return this.element.attr('transform', func);
  };

  Group.prototype.append = function(child) {
    return child.build(this.element);
  };
  ...
  Group.prototype.all = function(width, height) {
    ...
    this.element.data()[0].fixed = true;
    this.force = d3.layout.force().nodes(this.element.data()).size([(width / 12) * 10, height]).gravity(0.2).charge(this.charge).on('tick', tick).on('end', end).start();
    this.element.call(this.force.drag);
    return this;
  };

  Group.prototype.boundary = function() {
    var data, x, y;
    data = this.element.data().slice(1);
  ...
  Group.prototype.byCategory = function(width, height) {
    ...
    return this.element.transition().duration(3000).attr('transform', (function(_this) {
...
View = (function() {
  ...
  View.prototype.render = function(tab) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      row = ref[i];
      row.x = Math.random() * this.width;
      row.y = Math.random() * this.height;
    }
