...
Circle = (function() {
  ...
  Circle.radiusScale = d3.scale.linear();
...
Group = (function() {
  function Group(selected, data) {
    ...
    this.element = this.nodes.enter().append('g').attr('transform', this.transform).attr('data-toggle', 'popover').attr('title', this.title).attr('data-content', this.dataContent).attr('href', this.link).attr('data-placement', 'bottom');
  }

  Group.prototype.transform = function(data, index) {
    return "translate(" + data.x + "," + data.y + ")";
  };

  Group.prototype.title = function(data) {
    if (data.title != null) {
      return data.title;
    }
  };

  Group.prototype.link = function(data) {
    if (data.link != null) {
      return data.link;
    } else {
      return '#';
...
Row = (function() {
  function Row(record) {
    ...
    this.value = Row.weightScale(this.weight) + 1;
    this.score = [];
...
Text = (function() {
  ...
  Text.prototype.build = function(parent) {
    return this.element = parent.append('text').attr('dx', 0).attr('dy', 5).attr('text-anchor', 'middle').attr('opacity', 0).text(this.text);
  };
  ...
  Text.prototype.text = function(data) {
    if (data.weight > 0) {
      return data.weight;
    } else {
      return null;
...
View = (function() {
  ...
  View.prototype.render = function(data) {
    var circle, group, pack, text;
    pack = d3.layout.pack().sort(null).size([this.width, this.height]);
    group = new Group(this.element.selectAll('.node'), pack.nodes(data));
    circle = new Circle();
    text = new Text();
    group.append(circle);
    group.append(text);
...
table = new Table('data.csv', function() {
  var nodes;
  nodes = new Hierarchy(table).group('group').name('title').nodes();
  view.render(nodes);
  ...
  $('g').hover((function() {
    ...
    return $circle.data('previous-stroke', $circle.attr('stroke')).data('previous-stroke-width', $circle.attr('stroke-width')).attr('stroke', 'black').attr('stroke-width', 3);
  }), (function() {
    var $circle;
    $circle = $(this).find('circle');
    return $circle.attr('stroke', $circle.data('previous-stroke')).attr('stroke-width', $circle.data('previous-stroke-width'));
  }));
